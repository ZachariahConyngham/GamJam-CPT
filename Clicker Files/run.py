...
while True:
                ...
                if var.page == 0 and var.selectcol == 0 and var.select != -1:
                    if var.selected == False:
                        ...
                        if var.money >= var.generators[var.placeNames[var.select]][
                            ...
                            var.gn[var.select] += 1
                if var.page == 1 and var.selectcol == 1 and var.select != -1:
                    func.buyupgrade()
                if (
    ...
    for i in range(len(var.gn)):  # calculating money increase and sanity loss
        mult = 1
        var.tMpS += var.MpS[i] * var.gn[i] * ((var.upgBought[i] + 1) * 2)

        var.money += var.MpS[i] * var.gn[i] * ((var.upgBought[i] + 1) * 2) * 0.02
